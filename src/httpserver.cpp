...
boost::unique_future<bool> threadResult;
#else
...
#ifdef _WIN32
    // Boost.Thread doesn't have packaged_task, use a simpler approach
    threadHTTP = boost::thread(ThreadHTTP, eventBase, eventHTTP);
#else
...
#ifdef _WIN32
        // On Windows with Boost, just wait for the thread without timeout
        threadHTTP.join();
#else
