...
// Hardcoded seed node IPs removed — all original nodes are dead (circa 2013-2014)
unsigned int pnSeed[] = {};
...
bool BindListenPort(const CService &addrBind, string& strError)
{
    ...
    if (::bind(hListenSocket, (struct sockaddr*)&sockaddr, len) == SOCKET_ERROR)
    {
        ...
        if (nErr == WSAEADDRINUSE)
            strError = strprintf(_("Unable to bind to %s on this computer. Blakecoin is probably already running."), addrBind.ToString().c_str());
        else
